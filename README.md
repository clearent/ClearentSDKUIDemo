![Logo of the project](TODO)

# Clearent SDK UI Java
...
fun ClearentWrapper.initializeSDK(
    ...
    boolean enhancedMessages // should always be set to true, and should stay this way because ClearentSDKActivity does not support legacy messaging.
)
```
...
Intent intent = Intent(requireContext(), ClearentSDKActivity.class); // Use this intent to start 
the activity
...
```
intent.putExtra(
    ClearentSDKActivity.Companion.CLEARENT_ACTION_KEY,
    ClearentSDKActivity.Companion.CLEARENT_ACTION_PAIR
)

// If you want to show hints for the pairing process you should also do this:
intent.putExtra(
    ClearentSDKActivity.Companion.CLEARENT_SHOW_HINTS,
    true
)
```
...
```
intent.putExtra(
    ClearentSDKActivity.Companion.CLEARENT_PAYMENT_METHOD,
    (Parcelable) PaymentMethod // Where PaymentMethod is an enum with the values "CARD_READER" and "MANUAL_ENTRY"
)
```
...
intent.putExtra(
    ClearentSDKActivity.Companion.CLEARENT_ACTION_KEY,
    ClearentSDKActivity.Companion.CLEARENT_ACTION_TRANSACTION
)
intent.putExtra(
    ClearentSDKActivity.Companion.CLEARENT_PAYMENT_INFO_KEY,
    (Parcelable) paymentInfo  // Where paymentInfo is of type PaymentInfo and requires the amount field to be 
                              // different than null.
)
```
...
```
intent.putExtra(ClearentSDKActivity.Companion.CLEARENT_SHOW_SIGNATURE, false)
```
...
intent.putExtra(
    ClearentSDKActivity.Companion.CLEARENT_ACTION_KEY,
    ClearentSDKActivity.Companion.CLEARENT_ACTION_DEVICES
)
```
...
intent.putExtra(
    ClearentSDKActivity.Companion.CLEARENT_ACTION_KEY,
    ClearentSDKActivity.Companion.CLEARENT_ACTION_SETTINGS
)
```
...
interface OfflineStatusListener {
    fun onOfflineStatusChanged(offlineStatus: OfflineStatus)
...
```
void addOfflineStatusListener(OfflineStatusListener listener)
void removeOfflineStatusListener(OfflineStatusListener listener)
```
...
our [Example](https://github.com/clearent/ClearentSDKUIDemo/tree/Java) for full customization.
...
[Java EXAMPLE](https://github.com/clearent/ClearentSDKUIDemo/tree/Java) of the ClearentSDKActivity
integration.
...
public class App extends Application {
    ...
    private fun initClearentWrapper() = clearentWrapper.initializeSDK(
        ...
        baseUrl, // replace with the one received
        publicKey, // replace with the one received
        apiKey, // replace with the one received
        offlineModeConfig, // pass null if you don't want to enable offline mode
        enhancedMessages // should always be set to true, SDK UI does not support legacy messaging
    )
}
```
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        clearentWrapper.setListener(ClearentDataSource.INSTANCE);
    }

    private void startSdkUiActivity() {
        final Intent intent = Intent(this, ClearentSDKActivity.class);
        // Choose the action you want to take
        final ClearentAction clearentAction = ClearentAction.Pairing() ;
